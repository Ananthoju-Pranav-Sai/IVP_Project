import numpy as np
import torch
def maxpool_4d(tensor, kernel_shape, stride_shape):
    # Get dimensions of the input tensor
    b, c, d, h, w = tensor.shape

    # Get dimensions of the kernel
    kc, kd, kh, kw = kernel_shape

    # Get dimensions of the stride
    sc, sd, sh, sw = stride_shape

    # Calculate output tensor dimensions
    
    oc = int(np.floor((c - kc) / sc) + 1)
    od = int(np.floor((d - kd) / sd) + 1)
    oh = int(np.floor((h - kh) / sh) + 1)
    ow = int(np.floor((w - kw) / sw) + 1)

    # Create an empty output tensor
    output = torch.zeros((b, oc, od, oh, ow))

    # Apply maxpooling to each channel of the input tensor
    for i in range(b):
        for j in range(oc):
            for k in range(od):
                for l in range(oh):
                    for m in range(ow):
                        # Get the current pooling window
                        window = tensor[i, j*sc:j*sc+kc, k*sd:k*sd+kd, l*sh:l*sh+kh, m*sw:m*sw+kw]

                        # Compute the maximum value in the window
                        window_max = torch.max(window)

                        # Store the maximum value in the output tensor
                        output[i, j, k, l, m] = window_max
...
def render2D(opt, XYZid, ML, renderTrans):
    ...
    newiZMLCnt = max_pool4d(upNewiZMLCnt,ksize=[1,opt.upscale,opt.upscale,1],
												 strides=[1,opt.upscale,opt.upscale,1]) # [BN,H,W,3]
		newiZMLCnt = torch.reshape(newiZMLCnt,[opt.batchSize,opt.novelN,opt.H,opt.W,3])
    ...
    #                                     padding=0)
		...
		newML = max_pool4d(upNewML,ksize=[1,opt.upscale,opt.upscale,1],
									   strides=[1,opt.upscale,opt.upscale,1]) # [BN,H,W,1]
